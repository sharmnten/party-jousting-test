...
CharacterBody2D handles swept movement against static boundaries. A separate combat resolver takes all players' pre-movement positions and velocities, resolves swept circle contacts once per unordered pair, classifies stomps using contact normal, relative velocity and upper-body contact, and separates overlaps. Player bodies are visible on layer 1 but omit layer 1 from their engine movement mask so contact damage is resolved in exactly one place. Dead players immediately leave combat.
